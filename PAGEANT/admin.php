...
$nocacheHeadersSent = headers_sent();
if (!$nocacheHeadersSent) {
	// Strong no-cache headers for admin to avoid stale content on reload/open
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'add_criteria') {
        ...
        $name = strtoupper(trim(get_post('criteria_name')));
	...
	if ($action === 'edit_criteria') {
        ...
        $name = strtoupper(get_post('criteria_name'));
		$percentage = (int)get_post('criteria_percentage');
		$year = get_post('criteria_year');
	...
	if ($action === 'add_judge') {
        ...
        $name = strtoupper(get_post('judge_name'));
        $judge_number = strtoupper(get_post('judge_number'));
		if ($year && $name && $judge_number) {
			// Client-side handles duplicate checking; proceed to insert
	...
	if ($action === 'edit_judge') {
		$judge_id = (int)get_post('judge_id');
        $judge_number = strtoupper(get_post('judge_number'));
        $judge_name = strtoupper(get_post('judge_name'));
		if ($judge_id && $judge_number && $judge_name) {
			// Client-side handles duplicate checking; proceed to update
...
<head>
	...
	<style>
		...
		/* Force uppercase for all textual inputs in the UI */
...
function confirmTopEnd(options) {
    ...
    return Swal.fire({ position: 'center', toast: false, title, text, icon, showCancelButton: true, confirmButtonText, cancelButtonText });
}
...
function refreshJudgesSection(year) {
	...
	const previous = card ? card.outerHTML : '';
	const doFetch = () => fetch('admin.php?year=' + encodeURIComponent(year) + '&t=' + Date.now(), { cache:'no-cache' })
		.then(r => r.text())
		.then(html => {
			const parser = new DOMParser();
			const doc = parser.parseFromString(html, 'text/html');
			const freshCard = doc.getElementById('judgesCard');
			if (freshCard && card) {
				card.outerHTML = freshCard.outerHTML;
				return true;
			}
			// Fallback: replace only tbody rows if card not found
			const freshBody = doc.querySelector('#judgesTableBody');
			const localBody = document.querySelector('#judgesTableBody');
			if (freshBody && localBody) {
				localBody.innerHTML = freshBody.innerHTML;
				return true;
			}
			return false;
		});

	return doFetch().then(updated => {
		if (!updated) return;
		// If nothing visually changed, retry shortly once (handles very fast DB commit timing)
		const newCard = document.getElementById('judgesCard');
		const nowHtml = newCard ? newCard.outerHTML : '';
		if (previous === nowHtml) {
			setTimeout(() => { doFetch().catch(() => {}); }, 180);
		}
	}).catch(() => {});
}
...
function judgeNameExistsLocal(judgeName) {
	...
	for (const row of rows) {
		// Judge name is displayed in the second column
		const el = row.querySelector('td:nth-child(2) .fw-semibold');
		if (!el) continue;
...
function judgeNumberExistsLocal(judgeNumber) {
	...
	for (const row of rows) {
		// Judge number is displayed in the first column
		const el = row.querySelector('td:nth-child(1) .fw-semibold');
		if (!el) continue;
		const val = el.textContent.trim().toLowerCase();
		if (val === target) return true;
	}
...
function submitCriteriaAjax(e) {
	...
	const name = form.querySelector('input[name="criteria_name"]').value.trim().toUpperCase();
	...
	// Ensure the submitted value is uppercase
	form.querySelector('input[name="criteria_name"]').value = name;
...
function submitJudgeAjax(e) {
    ...
    const judgeNumber = form.querySelector('input[name="judge_number"]').value.trim().toUpperCase();
	const judgeName = form.querySelector('input[name="judge_name"]').value.trim().toUpperCase();
    const numberExists = judgeNumberExistsLocal(judgeNumber);
	const nameExists = judgeNameExistsLocal(judgeName);
	if (numberExists || nameExists) {
		if (numberExists && nameExists) {
			toast(`Judge number '${judgeNumber}' and Judge name '${judgeName}' already exist for this year.`, 'error');
		} else if (numberExists) {
			toast(`Judge number '${judgeNumber}' already exists for this year.`, 'error');
		} else {
			toast(`Judge name '${judgeName}' already exists for this year.`, 'error');
		}
		return false;
	}
    // Ensure uppercase values are submitted
    form.querySelector('input[name="judge_number"]').value = judgeNumber;
    form.querySelector('input[name="judge_name"]').value = judgeName;
    const payload = new URLSearchParams(new FormData(form));
    payload.append('ajax', '1');
    fetch('admin.php?year=' + encodeURIComponent(year), {
...
function saveCriteriaEdit(id) {
    const name = document.getElementById('editCriteriaName').value.trim().toUpperCase();
...
function clearCandidates() {
    confirmTopEnd({
        ...
        text: 'Are you sure you want to clear all candidates? This will permanently delete all candidate data for this year.',
        icon: 'warning',
					...
					<div class="modal-body">
						<form id="editJudgeForm">
							<div class="row g-3">
								...
								<div class="col-12">
									<label class="form-label fw-semibold">Judge Name</label>
                                    <input type="text" class="form-control" id="editJudgeName" value="${name}" required data-original-name="${name}">
								</div>
							</div>
						</form>
					</div>
...
function saveJudgeEdit(id) {
    const judgeNumber = document.getElementById('editJudgeNumber').value.trim().toUpperCase();
    const judgeName = document.getElementById('editJudgeName').value.trim().toUpperCase();
    const originalNumber = document.getElementById('editJudgeNumber').getAttribute('data-original-number') || '';
    const originalName = document.getElementById('editJudgeName').getAttribute('data-original-name') || '';
    if (!judgeNumber) { toast('Please enter a judge number', 'error'); return; }
    if (!judgeName) { toast('Please enter a judge name', 'error'); return; }
    const numberChanged = judgeNumber.toLowerCase() !== originalNumber.toLowerCase();
    const nameChanged = judgeName.toLowerCase() !== originalName.toLowerCase();
    const numberDup = numberChanged && judgeNumberExistsLocal(judgeNumber);
    const nameDup = nameChanged && judgeNameExistsLocal(judgeName);
    if (numberDup || nameDup) {
        if (numberDup && nameDup) {
            toast(`Judge number '${judgeNumber}' and Judge name '${judgeName}' already exist for this year.`, 'error');
        } else if (numberDup) {
            toast(`Judge number '${judgeNumber}' already exists for this year.`, 'error');
    ...
    payload.append('judge_id', id);
    // Ensure uppercase values are sent
