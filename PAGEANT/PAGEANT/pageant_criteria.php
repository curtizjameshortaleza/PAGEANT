		...
		<?php else: ?>
			<?php if ($all_done): ?>
				...
				<div style="margin-bottom:16px;">
					<button onclick="window.location.href='pageant.php'" class="btn">Return to Home</button>
				</div>
			<?php endif; ?>
            ...
            try {
                ...
                if (res.ok && (data === null || data.ok)) {
                    ...
                    if (criteriaId) {
                        ...
                        // AFTER marking this tab, check if all tabs are completed.
                        const allTabs = Array.from(document.querySelectorAll('.tabs .tab'));
                        const allCompleted = allTabs.length > 0 && allTabs.every(t => !!t.querySelector('.badge'));
						if (allCompleted) {
							// scroll to header so the toast (if any) appears visually over the top header
							const header = document.querySelector('.header-container');
							if (header) header.scrollIntoView({ behavior: 'smooth', block: 'start' });

							// reload immediately (no timers or extra message)
							location.reload();
							return true;
						}

                        // navigate to next tab (only if not all completed)
                        const tabs = allTabs;
                        ...
                        if (next) {
                            // request scroll when moving via submit-next actions
                            window._doScrollOnNext = true;
                            next.click();
                        }
                    }
...
document.addEventListener('DOMContentLoaded', function(){
	...
	function showPane(selector) {
		...
		// prefer scrolling the active tab button (acts like "this"), fallback to pane
		const activeBtn = document.querySelector('.tabs .btn.active');
		if (activeBtn) {
		...
		// If all criteria are submitted, ensure header is scrolled into view
		try {
			const allTabs = Array.from(document.querySelectorAll('.tabs .tab'));
			const allCompleted = allTabs.length > 0 && allTabs.every(t => !!t.querySelector('.badge'));
			const successAlert = document.querySelector('.alert-success');
			const alertIndicatesAll = successAlert && /All Criteria Submitted|Completed/i.test(successAlert.textContent);
			if (allCompleted || alertIndicatesAll) {
				const header = document.querySelector('.header-container');
				if (header) header.scrollIntoView({ behavior: 'smooth', block: 'start' });
			}
		} catch (err) { /* noop */ }
